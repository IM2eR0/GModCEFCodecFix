...
# print(colored("\t1. Teas Official 香港节点 #2（100Mbps）", "light_green"))
# print(colored("\t2. 紫枫堂镜像节点", "light_green"))
serverid = 1
	...
	try:
		...
		if "4000" in configVal["apps"]:
			if gmodSteamLibraryPath == None:
				...
				sys.exit(colored("错误：侦测到多个包含 GMod 的 Steam Libraries！一个 Steam Library 可能已经损坏。" + contactInfo + "ec(51)", "red"))
	except (FileNotFoundError, ValueError):
		continue

# if DEBUGMODE:print("\n\tDEBUG >", steamLibraryFoldersConfig or "NONE","\n")

if gmodSteamLibraryPath:
	print("\n已发现的Steam库：\n" + gmodSteamLibraryPath)
else:
	sys.exit(colored("错误: 未发现包含 GMod 的 Steam 库！" + contactInfo + "ec(52)\n", "red"))
